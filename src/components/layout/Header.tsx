...
export function Header() {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/5 bg-black/95 backdrop-blur-sm">
        ...
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-3 md:px-6">
          <Link to="/" className="shrink-0" onClick={() => setOpen(false)}>
            <img src={ASSETS.logo} alt="Silversant" className="h-9 md:h-10" />
          </Link>
            ...
            <nav className="flex h-full flex-col justify-center px-8 pt-20">
              {NAV.map((item, i) => (
                ...
                >
                  <Link
                    ...
                    className="block border-b border-white/5 py-5 font-display text-3xl font-medium uppercase tracking-wider text-white"
                  >
                    {item.label}
                  </Link>
                </motion.div>
              ...
              >
                <Link
                  to="/contact"
                  onClick={() => setOpen(false)}
                  className="inline-flex w-full items-center justify-center bg-accent px-6 py-3.5 font-display text-xs font-medium uppercase tracking-[0.18em] text-black hover:bg-accent-dim"
                >
                  Reserveren
                </Link>
              </motion.div>
